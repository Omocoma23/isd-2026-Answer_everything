...
def build_models():
    ...
    PLAN_CODE_RE = re.compile(r"^[0-9xX]{8}$")
    ...
    class Course(BaseModel):
        ...
        def _code_format(cls, v: str) -> str:
            ...
            if not CODE_RE.match(v):
                raise ValueError(
                    f"รหัสวิชาต้องเป็นตัวเลข 8 หลัก แต่ได้ '{v}'"
                )
    ...
    class PlanItem(BaseModel):
        ...
        def _code_format(cls, v: str) -> str:
            ...
            if not PLAN_CODE_RE.match(v):
                raise ValueError(
                    f"รหัสในแผนต้องยาว 8 ตัว และใช้ตัวเลขหรือ x แต่ได้ '{v}'"
                )
...
def _lab7b_plan_codes(value: Any) -> list[str]:
    """
    อ่านรหัสที่ใช้ในแผนการศึกษา
    รองรับทั้งรหัสจริงและ placeholder เช่น
    06026240, 06026xxx, 90644xxx, 9064xxxx, xxxxxxxx
    """
    text = str(value or "")
...
def convert_lab7b(data: dict, *, program_id: str | None = None,
    ...
    for index, src in enumerate(data.get("courses") or []):
        ...
        # codes = รหัสวิชาจริง ใช้สร้าง course
        codes = _lab7b_codes(raw_code)
        # plan_codes = รหัสที่อยู่ในแผน รองรับ x
        plan_codes = _lab7b_plan_codes(raw_code)
        if not plan_codes:
            warnings.append(
                f"courses[{index}] รหัสไม่อยู่ในรูปแบบที่รองรับ: {raw_code!r}"
            )
            continue
        # wildcard ไม่ใช่รายวิชาจริง จึงไม่สร้างใน course
        # แต่ต้องเก็บไว้ใน plan เพื่อให้นับหน่วยกิตถูก
        if not codes:
            skipped_wildcards += 1
            warnings.append(
                f"courses[{index}] {raw_code!r}: "
                f"เก็บไว้ใน plan แต่ไม่สร้าง course"
            )
        try:
        ...
        if not (1 <= year <= 8 and 1 <= semester <= 3):
            ...
            has_wildcard = any("x" in code.lower() for code in plan_codes)

            # กรณี A หรือ B เช่น 06026259 หรือ 06026260
            # ต้องนับหน่วยกิตเพียงครั้งเดียว
            if len(plan_codes) > 1:
                alt_group = f"lab7b_alt_{index}"

            # wildcard แต่ละแถวคือช่องวิชาเลือกหนึ่งช่อง
            # ต้องให้แต่ละแถวมี group ของตัวเอง
            elif has_wildcard:
                alt_group = f"lab7b_slot_{index}"
            ...
            notes = [
                str(x).strip()
                for x in (
                    src.get("name_th") if has_wildcard else None,
                    src.get("category"),
                    src.get("type"),
                    src.get("note"),
                )
...
def verify_db(conn: sqlite3.Connection) -> list[dict]:
    ...
    bad = conn.execute("""
        ...
        SELECT code FROM plan_item
        WHERE LENGTH(code) <> 8
        OR LOWER(code) GLOB '*[^0-9x]*'
    """).fetchall()

    add(
        "CHK3",
        "รหัสวิชาและช่องวิชาในแผนมีรูปแบบถูกต้อง",
        not bad,
        "ผิดรูปแบบ: " + ", ".join(r["code"] for r in bad[:8])
        if bad else "ถูกต้องทุกรายการ"
    )
...
SQL: SELECT code FROM plan_item WHERE year=1 AND semester=1 ORDER BY code

คำถาม: วิชาแคลคูลัส 1 มีรหัสอะไร
SQL: SELECT code FROM course WHERE name_th LIKE '%แคลคูลัส 1%'

คำถาม: วิชาระบบฐานข้อมูลขั้นสูงมีรหัสอะไร
SQL: SELECT code FROM course WHERE name_th LIKE '%ระบบฐานข้อมูลขั้นสูง%'

คำถาม: วิชา 06026243 มีกี่หน่วยกิต
SQL: SELECT credits FROM course WHERE code='06026243'

คำถาม: วิชา 06026250 มีกี่หน่วยกิต
SQL: SELECT credits FROM course WHERE code='06026250'
...
- ถ้าถามว่า "เรียนวิชาอะไรบ้าง" ให้ SELECT code จาก plan_item
- ถ้าถามหารหัสวิชาจากชื่อวิชา ให้ SELECT code FROM course และค้นใน name_th
- การค้นชื่อวิชาภาษาไทยให้ใช้ LIKE '%ชื่อวิชา%' แทนการเทียบแบบ = เพื่อรองรับข้อความที่มีความแตกต่างเล็กน้อย
- ถ้าถามข้อมูลของ "รายวิชา" เช่น ชื่อวิชา หน่วยกิต ชั่วโมงเรียน ให้ query ตาราง course
- ใช้ plan_item เฉพาะคำถามเกี่ยวกับแผนการศึกษา เช่น ปีไหน เทอมไหน เรียนวิชาอะไร หรือเรียนกี่หน่วยกิตในเทอมนั้น
